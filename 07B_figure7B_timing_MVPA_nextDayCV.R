...
# Figure 6B: MVPA timing (Pre/Post) -> next-day CV%
# Requires dat_timing, plus cgm_daily_var (or cgm_with_shift to rebuild)
...
if (!"next_day_cv" %in% names(dat_timing)) {
  dat_timing <- dat_timing %>%
    left_join(
      cgm_daily_var %>%
        mutate(id = toupper(trimws(as.character(id))), date = as.Date(date)) %>%
        select(id, date, next_day_cv = cv_percent),
...
dat_long <- dat_timing %>%
  ...
  select(id, mvpa_pre, mvpa_post, next_day_cv) %>%
...
fit <- lmer(next_day_cv ~ mvpa_min * timing + (1|id), data = dat_long, REML = FALSE)
...
fit_no_timing <- lmer(next_day_cv ~ mvpa_min + (1|id), data = dat_long, REML = FALSE)
...
ann <- dat_long %>%
  group_by(timing) %>%
  summarise(x = quantile(mvpa_min, 0.05, na.rm = TRUE), y = max(next_day_cv, na.rm = TRUE), .groups = "drop") %>%
  left_join(slopes, by = "timing")

p <- ggplot(dat_long, aes(x = mvpa_min, y = next_day_cv)) +
  ...
  labs(x = "MVPA in window (min)", y = "Next-day glucose variability (CV%)") +
  thesis_theme() +
  theme(legend.position = "none")

print(p)
ggsave("outputs/Figure6B_timing_MVPA_nextDayCV.pdf", p, width = 9, height = 4.8)
write.csv(slopes, "outputs/Figure6B_panel_stats.csv", row.names = FALSE)
write.csv(data.frame(
  test = "Timing + interaction (LRT)",
  chisq = lrt$Chisq[2],
  df = lrt$Df[2],
  p_value = lrt$`Pr(>Chisq)`[2]
), "outputs/Figure6B_overall_stats.csv", row.names = FALSE)
